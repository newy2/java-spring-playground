...
> 메서드 참조의 파라미터와 리턴 타입이 동일하다면, 다른 타입의 `FunctionalInterface`(예: ToIntBiFunction, Comparator) 변수에 메서드 참조를 할당할 수 있다. 이 규칙으로 기존의 Java API(예: Collection)에서 메서드 참조와 람다 표현식을 사용할 수 있다.
